...
func TestRFC8365_MultiRoutes(t *testing.T) {
	tests := []struct {
		...
		{
			// Known/unknown/known proves the skip path advances p by exactly l —
			// if `p += l` were wrong the trailing known route would fail to parse
			// or land on misaligned bytes.
			name: "known route, unknown type, known route — trailing known still decodes",
			input: []byte{
				// Type 5 IP Prefix Route (length 0x22 = 34)
				0x05, 0x22, 0x00, 0x01, 0x0A, 0x22, 0x04, 0x01, 0x00, 0x03,
				0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
				0x00, 0x00, 0x00, 0x00, 0x18, 0x0A, 0x0A, 0x0A, 0x00,
				0x00, 0x00, 0x00, 0x00, 0x00, 0x03, 0xFC,
				// Unknown type 0x99, length 4 — must be skipped
				0x99, 0x04, 0xDE, 0xAD, 0xBE, 0xEF,
				// Type 5 IP Prefix Route again
				0x05, 0x22, 0x00, 0x01, 0x0A, 0x22, 0x04, 0x01, 0x00, 0x03,
				0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
