...
func UnmarshalEVPNNLRI(b []byte) (*Route, error) {
	...
	for p := 0; p < len(b); {
		...
		case 8:
			n.RouteTypeSpec, err = UnmarshalEVPNMcastLeave(b[p : p+l])
			if err != nil {
				return nil, err
			}
		case 9:
		...
		default:
			glog.V(4).Infof("skipping unknown EVPN route type %d at offset %d (payload offset %d, length %d, remaining %d)", n.RouteType, p-2, p, l, len(b)-p)
			skipped++
			p += l
			continue
		}
