...
func TestUnmarshalSegmentListSTLV_TypeJ(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			input:             append([]byte{byte(TypeJ), 42}, base42...),
			wantLocalIfaceID:  1,
			wantLocalIPv6:     localIPv6,
			wantRemoteIfaceID: 2,
			wantRemoteIPv6:    remoteIPv6,
		...
		},
		{
			...
			input:             append(append([]byte{byte(TypeJ), 58}, base42...), srv6SID...),
			wantLocalIfaceID:  1,
			wantLocalIPv6:     localIPv6,
			wantRemoteIfaceID: 2,
			wantRemoteIPv6:    remoteIPv6,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			typeJSeg, ok := sl.Segment[0].(TypeJSegment)
			if !ok {
				t.Fatal("Segment is not TypeJSegment")
			}

			if typeJSeg.GetLocalInterfaceID() != tt.wantLocalIfaceID {
				t.Errorf("LocalInterfaceID = %d, want %d", typeJSeg.GetLocalInterfaceID(), tt.wantLocalIfaceID)
			}
			localIPv6Got := typeJSeg.GetLocalIPv6Address()
			if len(localIPv6Got) != len(tt.wantLocalIPv6) {
				t.Fatalf("localIPv6Address length = %d, want %d", len(localIPv6Got), len(tt.wantLocalIPv6))
			}
			for i, b := range tt.wantLocalIPv6 {
			...
			if typeJSeg.GetRemoteInterfaceID() != tt.wantRemoteIfaceID {
				t.Errorf("RemoteInterfaceID = %d, want %d", typeJSeg.GetRemoteInterfaceID(), tt.wantRemoteIfaceID)
			}
			remoteIPv6Got := typeJSeg.GetRemoteIPv6Address()
			if len(remoteIPv6Got) != len(tt.wantRemoteIPv6) {
				t.Fatalf("remoteIPv6Address length = %d, want %d", len(remoteIPv6Got), len(tt.wantRemoteIPv6))
			}
			for i, b := range tt.wantRemoteIPv6 {
			...
			sidGot, hasSID := typeJSeg.GetSRv6SID()
			if tt.wantSRv6SID == nil {
				if hasSID {
					t.Error("Expected no SRv6 SID, but got one")
				}
			} else {
				if !hasSID {
					t.Error("Expected SRv6 SID, but got none")
				} else if len(sidGot) != len(tt.wantSRv6SID) {
					t.Fatalf("SRv6 SID length = %d, want %d", len(sidGot), len(tt.wantSRv6SID))
				} else {
					for i, b := range tt.wantSRv6SID {
						if sidGot[i] != b {
							t.Errorf("SRv6 SID[%d] = %d, want %d", i, sidGot[i], b)
						}
					}
				}
			}
