...
func TestIdentityFromPeerUpVariations(t *testing.T) {
	...
	t.Run("loc rib zero local address uses speaker as router and empty peer ip", func(t *testing.T) {
		ph := makePeerHeader(t, bmp.PeerType3, 0)
		msg := bmp.Message{
			PeerHeader: ph,
			Payload:    buildPeerUpMessage(t, "0.0.0.0"),
			SpeakerIP:  "10.2.1.3",
		}
	...
	t.Run("zero local address without speaker is preserved", func(t *testing.T) {
		ph := makePeerHeader(t, bmp.PeerType0, 0)
		msg := bmp.Message{
			PeerHeader: ph,
			Payload:    buildPeerUpMessage(t, "0.0.0.0"),
		...
		if got.RouterIP != "0.0.0.0" {
			t.Errorf("RouterIP = %q, want original zero local address", got.RouterIP)
		}
		if got.RouterHash != "" {
			t.Errorf("RouterHash = %q, want empty hash for unspecified router IP", got.RouterHash)
		}
	})
