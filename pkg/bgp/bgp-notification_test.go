...
func TestUnmarshalNotificationMessage(t *testing.T) {
	tests := []struct {
		...
		{
			name:  "valid",
			input: []byte{0, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 21, 3, 4, 0},
			expect: &NotificationMessage{
				Length:       21,
				Type:         3,
				ErrorCode:    4,
		...
		},
		{
			...
			input: []byte{0, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 24, 3, 5, 0, 10, 24, 0},
			expect: &NotificationMessage{
				Length:       24,
				Type:         3,
				ErrorCode:    4,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if err != nil {
				if !tt.fail {
					t.Fatalf("%v expected to succeed but failed", tt.name)
				}
			}
			if err == nil {
				if tt.fail {
