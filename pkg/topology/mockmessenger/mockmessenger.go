...
func NewMockMessenger(p processor.Messenger) (Srv, error) {
	return &mockMessenger{
		...
		store: map[int]*mockMsg{
			bmp.PeerStateChangeMsg: &mockMsg{
				msgs: [2][]byte{
...
func (m *mockMessenger) messenger() {
	peer := time.NewTicker(time.Second * 120)
	lsNode := time.NewTicker(time.Second * 30)
	lsLink := time.NewTicker(time.Second * 30)
